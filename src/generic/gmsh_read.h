...
namespace GMSH
{
  ...
  struct Volume
  {
    ...
    std::vector<int> surfaceTags = {
      defFlag, defFlag, defFlag, defFlag, defFlag, defFlag};
  };

  /*! \brief: This for the point entity of the entity mesh. */
  struct Vertex
  {
    int entityDim = defFlag, entityTag = defFlag, parametric_ = defFlag,
        numNodesInBlock = defFlag;
    int nodeTag = defFlag;
    double x = 0.0, y = 0.0, z = 0.0;
  ...
  struct Element
  {
    int entityDim = defFlag, entityTag = defFlag, elementType = defFlag,
        numElementsInBlock = defFlag;
    ...
    std::vector<int> hexTags = {
      defFlag, defFlag, defFlag, defFlag, defFlag, defFlag, defFlag, defFlag};
  ...
  struct Hexa
  {
    ...
    std::vector<int> nodesTag = {
      defFlag, defFlag, defFlag, defFlag, defFlag, defFlag, defFlag, defFlag};
    std::vector<int> edgesTag = {defFlag,
                                 defFlag,
                                 defFlag,
                                 defFlag,
                                 defFlag,
  ...
  std::vector<T> operator+(const std::vector<T>& lhs, const std::vector<T>& rhs)
  {
    ...
    {
      std::cerr << "Error: Vector sizes do not match for addition."
                << std::endl;
  ...
  std::vector<T> operator-(const std::vector<T>& lhs, const std::vector<T>& rhs)
  {
    if (lhs.size() != rhs.size())
    {
      std::cerr << "Error: Vector sizes do not match for subtraction."
                << std::endl;
      return std::vector<T>(); // Return an empty vector
    }
    std::vector<T> result(lhs.size());
  ...
  std::vector<T>& operator+=(std::vector<T>& lhs, const std::vector<T>& rhs)
  {
    if (lhs.size() != rhs.size())
    {
      throw std::invalid_argument(
        "Error: Vector sizes do not match for addition.");
    }
    for (size_t i = 0; i < lhs.size(); ++i)
    {
      lhs[i] += rhs[i];
  ...
  std::vector<T>& operator-=(std::vector<T>& lhs, const std::vector<T>& rhs)
  {
    ...
    {
      throw std::invalid_argument(
        "Error: Vector sizes do not match for subtraction.");
    }
    for (size_t i = 0; i < lhs.size(); ++i)
    {
      lhs[i] -= rhs[i];
  ...
  std::vector<T> operator^(const std::vector<T>& lhs, const std::vector<T>& rhs)
  {
    ...
    {
      throw std::invalid_argument(
        "Vectors must have the same dimensionality for cross product.");
    }
  ...
  public:
    ...
    virtual Node& getNode(size_t n)
    {
      return Nodes[n];
    }

    const std::vector<Edge>& getEdges() const;
    Edge& getEdge(size_t e)
    {
      return Edges[e];
    }

    const std::vector<Quad>& getQuads() const;
    Quad& getQuad(size_t q)
    {
      return Quads[q];
    }

    const std::vector<Hexa>& getHexas() const;
    Hexa& getHexa(size_t h)
    {
      return Hexas[h];
    }
