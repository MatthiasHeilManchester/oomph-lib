...
GMSH::Gmsh::Gmsh(const std::string& filename, bool verbose) : Verbose(verbose)
{
  ...
  if (Msh_file.is_open())
  {
    while (std::getline(Msh_file, line))
    {
      ...
      else if (line == "$Entities")
      {
        ...
        std::getline(Msh_file, line);
        std::istringstream(line) >> numPEntity >> numCEntity >> numSEntity >>
          numVEntity;
      ...
      else if (line == "$Nodes")
      {
        ...
        std::istringstream(line) >> numEntityBlocks >> numNodes >>
          Min_node_tag >> maxNodeTag;
        if (numNodes > numEntityBlocks) Vertices.resize(numNodes);
        else
          Vertices.resize(numEntityBlocks);

        // initialize the nodes vector [for later use in collectNodes() not
        // here]
        ...
        for (int i = 0; i < numEntityBlocks; ++i)
        {
          ...
          std::istringstream(line) >> Vertices[count].entityDim >>
            Vertices[count].entityTag >> Vertices[count].parametric_ >>
            Vertices[count].numNodesInBlock;
          ...
          for (int j = 0; j < numNodesInBlock; ++j)
          {
            ...
            Vertices[count + j].nodeTag =
              tmp - Min_node_tag; // to reset numbering to zero use [tmp -
                                  // minNodeTag];
          }
          // Read the coord.
          for (int j = 0; j < numNodesInBlock; ++j)
          {
            ...
            std::istringstream(line) >> Vertices[count + j].x >>
              Vertices[count + j].y >> Vertices[count + j].z;
            if (j > 0)
            {
              Vertices[count + j].entityDim = Vertices[count + j - 1].entityDim;
              Vertices[count + j].entityTag = Vertices[count + j - 1].entityTag;
              Vertices[count + j].parametric_ =
                Vertices[count + j - 1].parametric_;
              Vertices[count + j].numNodesInBlock =
                Vertices[count + j - 1].numNodesInBlock;
            }
          }

          // reset count
          if (numNodesInBlock == 0) count += 1;
          else
      ...
      else if (line == "$Elements")
      {
        ...
        std::istringstream(line) >> numEntityBlocks >> numElem >>
        ...
        for (int i = 0; i < numEntityBlocks; ++i)
        {
          ...
          std::istringstream(line) >> Elements[count].entityDim >>
            Elements[count].entityTag >> Elements[count].elementType >>
            Elements[count].numElementsInBlock;
          ...
          for (int j = 0; j < Elements[count].numElementsInBlock; ++j)
          {
            if (j > 0)
            {
              ...
              Elements[count + j].elementType =
                Elements[count + j - 1].elementType;
              Elements[count + j].numElementsInBlock =
                Elements[count + j - 1].numElementsInBlock;
            }
            std::getline(Msh_file, line);
            std::istringstream iss(line);
            ...
            if (Elements[count + j].elementType ==
                ElementType::Quadrilateral2ndOrder_)
            {
              // resize tags to accommodate the new element type
            ...
            if (Elements[count + j].elementType ==
                ElementType::Hexahedral2ndOrder_)
            {
              // resize tags to accommodate the new element type
  ...
  std::cout << "read " << filename << " file in " << duration.count() << " [s]."
            << std::endl;
}
...
/*!@brief renumberBCondition the boundaries. Gmsh gives the user the flexibility
 * to choose the numbering, however oomph-lib require the numbering starting
 * from 0.
 * */
...
void GMSH::Gmsh::collectNodes()
{
  for (auto& vertex : Vertices)
  {
    ...
    {
      if (tuple.entityTag == Nodes[vertex.nodeTag].entityTag &&
          tuple.EntityType == EntityType::Point_)
      {
        // because node has boundary clear its boundary set
        Nodes[vertex.nodeTag].boundaries.clear();
        Nodes[vertex.nodeTag].boundaries.insert(tuple.boundary.begin(),
                                                tuple.boundary.end());
      }
    }
  }
  ...
  if (Elements.back().elementType == ElementType::Hexahedral_)
  {
    ...
    for (auto& hex : Hexas)
    {
      if (!correctOrientation(hex))
      {
        ...
        if (Verbose) std::cout << "Inverting hex " << hex.hexaTag << std::endl;
  ...
  std::for_each(Elements.begin(),
                ...
                [&](const Element& element)
                {
                      ...
                      element.elementType == ElementType::Line2ndOrder_)
                  {
                    ...
                    for (auto& tuple : Tuples)
                    {
                      if (tuple.entityTag == edge.entityTag &&
                          tuple.EntityType == EntityType::Curve_)
                      {
                        auto it = edge.boundaries.find(defFlag);
                        edge.boundaries.erase(*it);
                        edge.boundaries.insert(tuple.boundary.begin(),
                                               tuple.boundary.end());
                        ...
                        for (auto& nodeTag : edge.nodesTag)
                        {
                          ...
                          Nodes[nodeTag].boundaries.insert(
                            tuple.boundary.begin(), tuple.boundary.end());
                        }
                      }
                    }
...
bool GMSH::Gmsh::collectQuads()
{
  ...
  for (auto const& element : Elements)
  {
        ...
        element.elementType == Quadrilateral2ndOrder_)
    {
      ...
      for (auto& tuple : Tuples)
      {
        if (tuple.entityTag == quad.entityTag &&
            tuple.EntityType == quad.entityDim)
        {
          ...
          for (auto& nodeTag : quad.nodesTag)
          {
            it = Nodes[nodeTag].boundaries.find(defFlag);
            Nodes[nodeTag].boundaries.erase(*it);
            Nodes[nodeTag].boundaries.insert(tuple.boundary.begin(),
                                             tuple.boundary.end());
          }
        }
      }
...
bool GMSH::Gmsh::collectHexas()
{
  ...
  for (auto& element : Elements)
  {
    ...
    if (element.elementType == Hexahedral_)
    {
      ...
      for (auto& tuple : Tuples)
      {
        if (tuple.entityTag == hexa.entityTag &&
            tuple.EntityType == hexa.entityDim)
        {
...
void GMSH::Gmsh::addMissingQuad(Hexa& hex)
{
  ...
  for (auto& face : faces)
  {
    if (!inList(face.nodesTag, Quads)) addInternalQuad(face.nodesTag);
...
void GMSH::Gmsh::addMissingEdge(Quad& quad)
{
  ...
  for (auto& line : lines)
  {
    if (!inList(line.nodesTag, Edges)) addInternalEdge(line.nodesTag);
  }
}
...
void GMSH::Gmsh::renumber()
{
  ...
  for (auto& edge : Edges)
  {
    for (int boundary : edge.boundaries)
    {
      ...
      if (it != edge.boundaries.end() && *it != defFlag)
      {
        ...
        edge.boundaries.insert(
          orderedBC[boundary]); // replace it with the new b.c.
      }
    }
  }
  ...
  if (getMeshDim() == 2)
  {
    ...
    for (auto& quad : Quads)
    {
      ...
      {
        quad.boundaries.insert(Edges[eTag].boundaries.begin(),
                               Edges[eTag].boundaries.end());
      }

      // Delete the defFlag if it exists
      if (quad.boundaries.size() > 1 &&
          quad.boundaries.find(defFlag) != quad.boundaries.end())
      {
  ...
  if (getMeshDim() == 3)
  {
        ...
        if (it != quad.boundaries.end() && *it != defFlag)
        {
          ...
          for (auto eTag : quad.edgesTag)
          {
            ...
            if (Edges[eTag].boundaries.size() > 1 &&
                Edges[eTag].boundaries.find(defFlag) !=
                  Edges[eTag].boundaries.end())
            {
    ...
    for (auto& hex : Hexas)
    {
      ...
      {
        hex.boundaries.insert(Quads[qTag].boundaries.begin(),
                              Quads[qTag].boundaries.end());
      }

      // Delete the defFlag if it exists
      if (hex.boundaries.size() > 1 &&
          hex.boundaries.find(defFlag) != hex.boundaries.end())
      {
...
bool GMSH::Gmsh::correctOrientation(Quad& quad)
{
  ...
  for (int i = 0; i < 4; ++i)
  {
    ...
    const double dotprod = (node2[0] - node1[0]) * (node3[1] - node2[1]) -
                           (node2[1] - node1[1]) * (node3[0] - node2[0]);
  ...
  {
    ordered = false;
    std::cerr << "quad " << quad.quadTag
              << " is not convex - errors may emerge!" << std::endl;
  }
  return ordered;
}
...
bool GMSH::Gmsh::correctOrientation(const Hexa& hex)
{
  ...
  // we just need the first and second face (I think: two faces check is
  // enough!)
...
std::vector<GMSH::Quad> GMSH::Gmsh::getFaces(const Hexa& hexa)
{
  ...
  faces[0].nodesTag = {
    hexa.nodesTag[3], hexa.nodesTag[2], hexa.nodesTag[1], hexa.nodesTag[0]};
  faces[1].nodesTag = {
    hexa.nodesTag[4], hexa.nodesTag[5], hexa.nodesTag[6], hexa.nodesTag[7]};
  faces[2].nodesTag = {
    hexa.nodesTag[0], hexa.nodesTag[1], hexa.nodesTag[5], hexa.nodesTag[4]};
  faces[3].nodesTag = {
    hexa.nodesTag[2], hexa.nodesTag[3], hexa.nodesTag[7], hexa.nodesTag[6]};
  faces[4].nodesTag = {
    hexa.nodesTag[0], hexa.nodesTag[4], hexa.nodesTag[7], hexa.nodesTag[3]};
  faces[5].nodesTag = {
    hexa.nodesTag[1], hexa.nodesTag[2], hexa.nodesTag[6], hexa.nodesTag[5]};
  return faces;
}
...
std::vector<double> GMSH::Gmsh::getArea(const std::vector<int>& points)
{
  ...
  if (points.size() == 3)
  {
    return 0.5 * ((Nodes[points[1]].coord - Nodes[points[0]].coord) ^
                  (Nodes[points[2]].coord - Nodes[points[0]].coord));
  }
  ...
  for (int pi = 0; pi < points.size(); ++pi)
  {
    ...
    const std::vector<double>& pNext =
      Nodes[points[(pi + 1) % points.size()]].coord;
...
const std::vector<GMSH::Node>& GMSH::Gmsh::getNodes() const
{
  return Nodes;
}

const std::vector<GMSH::Edge>& GMSH::Gmsh::getEdges() const
{
  return Edges;
}

const std::vector<GMSH::Quad>& GMSH::Gmsh::getQuads() const
{
  return Quads;
}

const std::vector<GMSH::Hexa>& GMSH::Gmsh::getHexas() const
{
  return Hexas;
}

unsigned GMSH::Gmsh::getBoundSize() const
{
  return boundaries.size();
}

unsigned GMSH::Gmsh::getNodesSize() const
{
  return Nodes.size();
}

unsigned GMSH::Gmsh::getEdgesSize() const
{
  return Edges.size();
}

unsigned GMSH::Gmsh::getQuadsSize() const
{
  return Quads.size();
}

unsigned GMSH::Gmsh::getHexasSize() const
{
  return Hexas.size();
}
...
unsigned GMSH::Gmsh::getMeshDim() const
{
  unsigned dim = 3;
  if (Elements.back().elementType == ElementType::Hexahedral_ ||
      Elements.back().elementType == ElementType::Hexahedral2ndOrder_)
  {
    dim = 3;
  }
...
int GMSH::Gmsh::getNodesPerElement() const
{
  return nodesPerElement;
}

int GMSH::Gmsh::getEdgesPerElement() const
{
  return edgesPerElement;
}

int GMSH::Gmsh::getFacesPerElement() const
{
  return facesPerElement;
}
...
void GMSH::Gmsh::nodesInfo()
{
  ...
  for (auto& node : Nodes)
  {
    std::cout << " tag: " << std::setw(width) << std::setprecision(precision)
              << node.nodeTag;
    ...
    {
      std::cout << std::setw(2) << std::setfill('0') << boundary << " ";
    }


    // std::cout << " E.Tag: " << std::setw(width) <<
    // std::setfill('0')<<node.entityTag; std::cout << " E.Dim: " <<
    // std::setw(width) << std::setfill('0')<<node.entityDim;

    std::cout << " Coord: "
              << "(" << std::setw(width) << std::setprecision(precision)
              << node.coord[0] << " " << std::setw(width)
              << std::setprecision(precision) << node.coord[1] << " "
              << std::setw(width) << std::setprecision(precision)
              << node.coord[2];
    std::cout << ")";
...
void GMSH::Gmsh::edgesInfo()
{
  ...
  for (auto& edge : Edges)
  {
    ...
    {
      std::cout << std::setw(2) << std::setfill('0') << node << " ";
    }
    std::cout << ")";
...
void GMSH::Gmsh::quadsInfo()
{
  ...
  for (auto& quad : Quads)
  {
    ...
    {
      std::cout << std::setw(2) << std::setfill('0') << edge << " ";
    }
    std::cout << ")";
...
void GMSH::Gmsh::hexasInfo()
{
  ...
  for (auto& hexa : Hexas)
  {
    std::cout << " tag: " << std::setw(2) << std::setfill('0') << hexa.hexaTag;
    ...
    {
      std::cout << std::setw(2) << std::setfill('0') << quadTag << " ";
    }
    std::cout << ")";
    ...
    {
      std::cout << std::setw(2) << std::setfill('0') << edgeTag << " ";
    }
    std::cout << ")";
