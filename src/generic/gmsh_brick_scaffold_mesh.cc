...
namespace oomph
{
  ...
  //======================================================================
  GmshBrickScaffoldMesh::GmshBrickScaffoldMesh(const std::string& file_name,
                                               bool verbose)
  {
      ...
      for (unsigned j = 0; j < n_local_node; j++)
      {
        ...
        if (!done[global_node_number])
        {
          ...
          for (int i = 0; i < bound[global_node_number].size(); ++i)
          {
            ...
            {
              Node_pt[global_node_number] =
                finite_element_pt(e)->construct_node(gmsh_to_oomph_node[j]);
            }
